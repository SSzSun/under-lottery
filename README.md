...
- 📝 **บันทึกรายการซื้อหวย** — รองรับเลข 1-3 หลัก, ประเภทบน/ล่าง/ลอยบน/ลอยล่าง, หวยกลุ่ม 5 หลัก
- 🔄 **โหมดหมุนเลข** — หมุนหน้า/หมุนท้าย สำหรับเลข 3 หลักประเภทล่าง
- 🚫 **เลขอั้น** — จัดการเลขที่ไม่รับแทงในแต่ละงวด (แยกตามงวด) + เปิด/ปิดขายแต่ละเลข
...
2. กรอกเลข (1-3 หลัก)
3. กด "เพิ่ม" → สถานะเริ่มต้นคือ **เปิดขาย** (สีเขียว)
4. **เปิด/ปิดขาย** — กดปุ่มสถานะใต้เลขเพื่อสลับ:
   - **เปิดขาย** (สีเขียว) → สามารถแทงได้
   - **ปิดขาย** (สีแดง) → ไม่สามารถแทงได้
5. ลบได้เฉพาะงวดปัจจุบัน (งวดอื่นจะมีไอคอน 🔒)
...
- เลข 5 หลัก → หวยกลุ่ม (ตัวเลขต้องไม่ซ้ำกัน, 1 ช่องเงิน)
- เลขเดียวกัน (เช่น 11, 222) → 1 ช่องเงิน
- โหมดหมุน → 1 ช่องเงิน
- เลขอั้น (ปิดขาย) → ไม่สามารถบันทึกได้
- เลขอั้น (เปิดขาย) → สามารถบันทึกได้ปกติ
...
- `number` (TEXT)
- `draw_id` (UUID, FK → draws)
- `is_open` (BOOLEAN, DEFAULT true) — false = ปิดขาย (อั้น), true = เปิดขาย
- `created_at` (TIMESTAMPTZ)
